...
r"""Qwen3.6-35B-A3B vision-language inference + optional INT4 quantization.

Qwen3.6-35B-A3B is the Qwen3.5-MoE-VL family (HF ``model_type = qwen3_5_moe``
when the composite carries a ``vision_config``). It pairs the hybrid linear/
full attention MoE text backbone (256 experts, 8 active, ~3B activated) with
the Qwen3-VL ViT, exported by mobius as a 3-model split: ``decoder`` (MoE
LM head over ``inputs_embeds``), ``vision_encoder`` (Qwen3-VL ViT),
``embedding`` (token embed + image-feature fusion).

ORT GenAI loads it under ``model.type = "qwen2_5_vl"`` because the 3-model
I/O contract matches; mobius selects ``Qwen35MoEVL3ModelCausalLMModel``
based on the presence of ``vision_config`` in the HF composite.

The decoder mixes DeltaNet linear-attention layers (recurrent state) with
full GQA layers (KV cache). ORT GenAI's built-in generator doesn't manage
DeltaNet recurrent state, so generation here uses manual ONNX Runtime
sessions with onnxruntime-genai providing only the tokenizer.
...
Usage::
    ...
    # With an image:
    python examples/qwen3_6_35b_a3b_ort_genai.py \\
        --image testdata/pipeline-cat-chonk.jpeg --save-to output/qwen3_6/

    # Reuse a pre-built model directory:
    python examples/qwen3_6_35b_a3b_ort_genai.py --model-dir output/qwen3_6/

    # Just build + save:
    python examples/qwen3_6_35b_a3b_ort_genai.py --save-to output/qwen3_6/

    # INT4-quantize the decoder with Olive (Q4_K_M, GPU-accelerated):
    python examples/qwen3_6_35b_a3b_ort_genai.py \\
        --save-to output/qwen3_6/ \\
        --quantize Q4_K_M --quantized-out output/qwen3_6-int4/
"""
...
def quantize_decoder_with_olive(
    src_dir: str,
    dst_dir: str,
    *,
    ...
    Copies the vision encoder, embedding model, tokenizer, and
    ``genai_config.json`` over unchanged — quantization only targets the
    text decoder, which is by far the largest sub-model. The resulting
    directory is a drop-in replacement for ``src_dir`` in ORT GenAI.

    Args:
        src_dir: full-precision build (the output of ``build_and_export``).
        dst_dir: destination directory.
        precision: ``"Q4_K_M"`` (k-quant, GPU-accelerated with cupy) or
            ``"NF4"`` (BitsAndBytes 4-bit NormalFloat, native C++).
        block_size: k-quant block size; ignored for NF4.

    Requires Olive 0.8+ (``pip install olive-ai`` or
    ``pip install -e /path/to/Olive``). For Q4_K_M, install
    ``cupy-cuda12x`` to get the 19-51x GPU speedup over CPU k-quant.
    """
    import shutil
    ...
    decoder_src = os.path.join(src_dir, "decoder", "model.onnx")
    decoder_dst_dir = os.path.join(dst_dir, "decoder")
    config = {
        "input_model": {"type": "OnnxModel", "model_path": decoder_src},
        "passes": {precision.lower(): pass_cfg},
        "output_dir": decoder_dst_dir,
    }

    print(f"Quantizing decoder ({precision}) -> {decoder_dst_dir} ...")
    olive_run(config)
    ...
    print(f"Quantized package ready at {dst_dir}")
    print(
        "(Note: ORT GenAI loads it the same way as the full-precision "
        "package; only the decoder weights changed.)"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "After building/loading the model, run Olive on the decoder to "
            "produce an INT4 quantized variant. Q4_K_M uses k-quant "
            "(GPU-accelerated when cupy is installed); NF4 uses BitsAndBytes "
            "NF4 (native C++, no GPU needed)."
        ),
    )
    parser.add_argument(
        "--quantized-out",
        metavar="DIR",
        default=None,
        help=("Output directory for --quantize. Defaults to <model-dir>-{Q4_K_M|NF4}/."),
    )
    ...
    if args.save_to:
        ...
        if args.quantize:
            out = args.quantized_out or f"{args.save_to.rstrip('/')}-{args.quantize}"
            quantize_decoder_with_olive(args.save_to, out, precision=args.quantize)
        return
