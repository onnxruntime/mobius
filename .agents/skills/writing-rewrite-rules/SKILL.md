...
description: >
  Use this skill when creating ONNX rewrite rules that transform parts of an
  ONNX model graph, such as replacing standard ops with custom or fused ops.
  Covers the onnxscript.rewriter RewriteRuleClassBase API, pattern matching
  with op functions, check/rewrite method conventions, file organization
  under src/mobius/rewrite_rules/, and testing patterns for verifying rule
  correctness.
---
