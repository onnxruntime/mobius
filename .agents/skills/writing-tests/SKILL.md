...
|-------|------|-----------------|-------------|
| **L1** | Graph builds | ONNX graph builds from a tiny synthetic config | `tests/_test_configs.py` + `tests/build_graph_*_test.py` |
...
python -m pytest tests/build_graph_*_test.py --fast

# Single model type
python -m pytest tests/build_graph_*_test.py -k "phi4mm"
...
```
tests/
├── build_graph_*_test.py       # L1: domain-split graph construction (no weights)
├── _build_graph_support.py     # shared L1 helpers and coverage inventory
...
Located in `tests/build_graph_*_test.py`. Uses tiny synthetic configs
...
`tests/_build_graph_support.py::_SPECIALIZED_TEST_MODEL_TYPES`.
