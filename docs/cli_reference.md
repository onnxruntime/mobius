...
| `--target-gguf PATH` | Exact target GGUF for a `dflash`/`eagle3` pair; emits target/draft graphs, cache namespaces, required shared-weight bridges, and a `runtime_unvalidated` direct-ORT manifest. |
| `--release` | Strip build-time debug and provenance metadata before saving while preserving functional `mobius.*` metadata. |
