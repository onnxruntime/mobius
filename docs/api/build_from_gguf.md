...
|---|---:|---|
| Architectures | 147 | graph verdicts: {'deferred': 88, 'rejected': 2, 'supported': 57}; importable: 55; quantized import: {'rejected': 10, 'supported': 137}; runtime: {'deferred': 145, 'rejected': 2} |
...
| `bloom` | — | none (no tensor mapping route) | not claimed | config=supported; tensor_map=deferred; graph=supported; runtime=deferred; quantized_import=rejected | mobius has no GGUF→HuggingFace tensor-name mapping for this architecture, so its weights cannot be routed into a graph. Build from the Hugging Face checkpoint with `mobius build` instead. |
...
| `codeshell` | — | none (fails before config extraction) | audited-direct-loader-conditional-union | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | CodeShell uses bias-bearing LayerNorm blocks, grouped fused/split QKV semantics, a sequential GELU FFN, and an architecture-specific embedding/output tie direction. GPT-NeoX similarity is not an exact tensor or graph contract. |
...
| `command-r` | — | none (fails before config extraction) | audited-direct-loader-conditional-union | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | Command-R uses one weight-only norm feeding attention and SwiGLU in parallel, conditional Q/K norms for large variants, tied output, and logit scaling. The canonical command-r and cohere2 GGUF contracts are distinct and cannot alias. |
...
| `ernie4_5` | — | none (fails before config extraction) | audited-direct-loader-conditional-union | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | ERNIE 4.5 requires exact fused-QKV and fused-gate/up converter splits plus an optional attention-output bias and ERNIE-specific position metadata. Similarity to Qwen/Llama is not a suffix-exact tensor or graph contract. |
...
| `eurobert` | — | none (fails before config extraction) | not claimed | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | EuroBERT uses pre-norm RMSNorm, RoPE, bias-free split Q/K/V attention, and a parallel SwiGLU FFN. Neither the post-norm BertModel nor ModernBertModel graph matches that architecture. |
...
| `openelm` | — | none (fails before config extraction) | not claimed | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | The pinned OpenELM graph requires per-layer Q/KV-head and feed-forward-width arrays, fused QKV with Q/K normalization, and mandatory tied embeddings. Those per-layer contracts cannot be scalarized into the current generic graph. |
| `orion` | — | none (fails before config extraction) | audited-direct-loader-conditional-union | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | Orion uses bias-bearing LayerNorm at attention, FFN, and output sites despite its source config naming rms_norm_eps. Mobius Llama uses RMSNorm; interpreting that metadata generically would build the wrong normalization. |
...
| `qwen` | — | none (fails before config extraction) | audited-direct-loader-conditional-union | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | Qwen-v1 requires NeoX-style RoPE, fused biased QKV, and a fused-width SwiGLU conversion contract. Mobius's HF qwen family registration is not evidence that the canonical GGUF qwen tensor layout is importable. |
...
| `qwen2vl` | — | none (fails before config extraction) | exact-direct-loader-conditional-union | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | The qwen2vl architecture is shared by Qwen2-VL, Qwen2.5-VL, and Qwen2.5-Omni converter paths whose clip companions use different projector strings and modalities. Exact M-RoPE sections, special tokens, merger dimensions, processor ordering, and target identity must select one complete package; a generic Qwen2 alias would erase those distinctions. |
...
| `refact` | — | none (fails before config extraction) | audited-direct-loader-conditional-union | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | Refact uses hard-coded ALiBi without RoPE, packed-KV and gate/up converter splits, optional projection/MLP biases, and a single KV head. Llama-like tensor names are not evidence for a compatible graph. |
...
| `starcoder` | — | none (fails before config extraction) | audited-direct-loader-conditional-union | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | StarCoder requires learned absolute positions, fused biased QKV, causal multi-query attention, bias-bearing LayerNorm/GELU blocks, and a one-head KV cache. StarCoder2 uses RoPE and is a different canonical GGUF architecture. |
...
| `xverse` | — | none (fails before config extraction) | audited-direct-loader-conditional-union | config=deferred; tensor_map=deferred; graph=deferred; runtime=deferred; quantized_import=supported | Xverse is Llama-shaped but requires converter-defined Q/K reverse permutations, including a GQA-specific K transform, before quantized packing. No exact Mobius metadata/tensor/value-transform recipe currently owns that contract. |
