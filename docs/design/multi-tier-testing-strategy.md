...
| **File** | `tests/build_graph_*_test.py` with configs from `tests/_test_configs.py` |
...
3. Run:  python -m pytest tests/build_graph_*_test.py -k "my_model"        → L1
...
| L1 | ✅ Full coverage (273) | 273 types | `build_graph_*_test.py` + `_SPECIALIZED_TEST_MODEL_TYPES` |
...
|-------|------------------|-------------|
| L1 | `_scan_l1_configs` | `tests/_test_configs.py::ALL_CONFIGS` + `tests/_build_graph_support.py::_SPECIALIZED_TEST_MODEL_TYPES` |
...
pytest tests/build_graph_*_test.py --fast

# L1: Single model
pytest tests/build_graph_*_test.py -k "qwen2"
...
pytest tests/build_graph_*_test.py --models "qwen2,llama"
...
- **L1 (Graph builds)**: Achieved by adding a tiny config to
  ...
  `build_graph_*_test.py` picks it up automatically.
...
5. Run `pytest tests/build_graph_*_test.py -k "<name>"` → L1
