...
class TestCLIBuild:
    ...
    def test_features_paged_cache_builds_paged_task(self):
        """--features paged-cache passes a paged CausalLMTask to build()."""
        with (
            tempfile.TemporaryDirectory() as tmpdir,
            mock.patch(
                "mobius._diffusers_builder._load_diffusers_pipeline_index",
        ...
        task = mock_build.call_args.kwargs.get("task")
        assert getattr(task, "_paged_cache", False) is True
