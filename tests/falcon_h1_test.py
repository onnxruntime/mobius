...
def test_falcon_h1_prefill_decode_logits_and_four_states_match_transformers() -> None:
    ...
    try:
        ...
        for layer, hf_state in enumerate(hf_prefill.past_key_values.layers):
            ...
            # transformers >=5.14 changed conv_states/recurrent_states from a tensor
            # to a dict keyed by state index; extract the tensor for either version.
            hf_conv_states = hf_state.conv_states
            if isinstance(hf_conv_states, dict):
                hf_conv_states = hf_conv_states[0]
            hf_recurrent_states = hf_state.recurrent_states
            if isinstance(hf_recurrent_states, dict):
                hf_recurrent_states = hf_recurrent_states[0]
            np.testing.assert_allclose(
                ort_prefill[f"present.{layer}.conv_state"],
                hf_conv_states[:, :, -(config.mamba_d_conv - 1) :].numpy(),
                rtol=1e-4,
                atol=1e-4,
            )
            np.testing.assert_allclose(
                ort_prefill[f"present.{layer}.ssm_state"],
                hf_recurrent_states.transpose(2, 3).numpy(),
                rtol=1e-3,
                atol=1e-3,
            )
