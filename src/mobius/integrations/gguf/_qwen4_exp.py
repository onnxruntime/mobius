...
def _payload_blocker(*, keep_quantized: bool) -> Qwen4ExpGGUFImportError:
    if keep_quantized:
        ...
        detail = (
            "per_layer_token_embd.weight expands to roughly 95 GiB by itself, "
    ...
    return Qwen4ExpGGUFImportError(
        ...
        f"{detail} No GGUF payload was downloaded or read. The exact header, "
...
def _expected_shapes(metadata: dict[str, Any]) -> dict[str, tuple[int, ...]]:
    ...
    ngram_heads = (
        int(metadata[f"{prefix}ple.ngram_size"]) - 1
    ) * int(metadata[f"{prefix}ple.heads_per_ngram"])
        ...
        shapes.update(
            {
                ...
                f"{root}.ffn_gate_inp_shexp.weight": (1, hidden),
            }
        )
...
def validate_qwen4exp_tensor_contract(
    ...
    *,
    ...
    expected_qtypes = {
        ...
        **{
            f"blk.{layer}.{projection}": qtype
            for layer in range(48)
            for projection, qtype in (
                ("ffn_gate_exps.weight", "IQ1_S"),
                ("ffn_up_exps.weight", "IQ1_S"),
                ("ffn_down_exps.weight", "IQ4_NL"),
            )
        },
        **{
            f"blk.{layer}.indexer.{projection}.weight": "BF16"
