...
class CausalLMTask(ModelTask):
    ...
    def build(
        ...
        # MLA attention: K/V heads equal q heads (no GQA reduction in latent space).
        # The ONNX Attention op is called with kv_num_heads=num_attention_heads,
        # so the KV cache must use num_attention_heads (not num_key_value_heads).
        use_mla = (config.qk_nope_head_dim is not None and config.qk_nope_head_dim > 0) or (
            config.qk_rope_head_dim is not None and config.qk_rope_head_dim > 0
        )
        num_kv_cache_heads = config.num_attention_heads if use_mla else config.num_key_value_heads
