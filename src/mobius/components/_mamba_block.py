...
Mamba2Block supports two multi-token modes:
    - Chunked SSD (default, ``flags.mamba_scan=True``): parallel within
      chunks, O(T·CS) per chunk.  Matches HF ``torch_forward``.
    - Single-token fallback (``flags.mamba_scan=False``): processes one
      token at a time.  Useful for debugging.
...
class Mamba2Block(nn.Module):
    ...
    Supports two multi-token modes controlled by ``flags.mamba_scan``:

    - **Chunked SSD** (default, ``mamba_scan=True``): processes all
    ...
    - **Single-token fallback** (``mamba_scan=False``): per-token
      recurrence.  Useful for debugging numerical issues.
    ...
    def forward(
        ...
        When ``flags.mamba_scan`` is True (default), uses the chunked
        SSD algorithm (parallel within chunks, O(T/CS) cross-chunk
        recurrence).  When False, falls back to a single-token path
        (seq_len must be 1).
