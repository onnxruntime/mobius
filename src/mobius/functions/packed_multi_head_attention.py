...
def packed_multi_head_attention() -> ir.Function:
    ...
    """
    # --- Graph inputs ---
    query_input = ir.Value(name="query")
    key_input = ir.Value(name="key")
    value_input = ir.Value(name="value")
    token_offset_input = ir.Value(name="token_offset")
    cumulative_sequence_length_input = ir.Value(name="cumulative_sequence_length")
    ...
    graph = ir.Graph(
        ...
        outputs=[],
        nodes=[],
        name="PackedMultiHeadAttention_body",
    ...
    return ir.Function(
        ...
        graph=graph,
        attributes={
            "num_heads": ir.Attr("num_heads", ir.AttributeType.INT, 1),
            "scale": ir.Attr("scale", ir.AttributeType.FLOAT, 1.0),
        },
    )
