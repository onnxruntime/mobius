    ...
    for descriptor in manifest.values():
        ...
        owned_elsewhere = (
            tuple(
                ...
                if other.name != descriptor.name and other.module_path
            )
...
def _canonical_component_parameter_keys(
    ...
    keys: set[str] = set()
    prefixes = {prefix for prefix in (descriptor.name, descriptor.module_path) if prefix}
...
def _route_component_weights(
    ...
    prefixes = {
        name: {prefix for prefix in (name, manifest[name].module_path) if prefix}
    ...
    def owner(key: str) -> str | None:
        ...
        if not matches:
            root_components = [
                name for name in component_names if not manifest[name].module_path
            ]
...
def _local_weight_module_path(
    ...
    for prefix in (descriptor.module_path, descriptor.name):
