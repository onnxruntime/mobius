...
class TestRenameVitWeight:
    ...
    def test_dinov2_mlp_fc2_to_down_proj(self):
        result = _rename_vit_weight(
            "dinov2.encoder.layer.1.mlp.fc2.bias"
        )
    ...
    def test_dinov2_layer_scale_skipped(self):
        assert (
            _rename_vit_weight("dinov2.encoder.layer.0.layer_scale1")
            is None
        )
        assert (
            _rename_vit_weight("dinov2.encoder.layer.1.layer_scale2")
            is None
        )
    ...
    def test_beit_prefix_strip(self):
        result = _rename_vit_weight(
            "beit.encoder.layer.0.attention.attention.query.weight"
        )
        assert result == "encoder.layer.0.self_attn.q_proj.weight"
    ...
    def test_beit_lambda_skipped(self):
        assert (
            _rename_vit_weight("beit.encoder.layer.0.lambda_1") is None
        )
        assert (
            _rename_vit_weight("beit.encoder.layer.0.lambda_2") is None
        )

    def test_beit_relative_position_bias_skipped(self):
        assert (
            _rename_vit_weight(
                "beit.encoder.layer.0.attention.attention."
...
class TestRenameVitWeightBatch:
    ...
    @pytest.mark.parametrize(
        "prefix", ["vit", "beit", "deit", "dinov2", "swin", "hiera"]
    )
