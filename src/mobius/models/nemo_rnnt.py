...
class RelPositionMultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        q = self._split_heads(op, self.linear_q(op, x))  # (B, T, h, d_k)
        k = self._split_heads(op, self.linear_k(op, x))
        v = self._split_heads(op, self.linear_v(op, x))
        # to (B, h, T, d_k)
...
class ConformerConvolution(nn.Module):
    ...
    def forward(self, op: OpBuilder, x: ir.Value) -> ir.Value:
        ...
        x = op.Mul(a, op.Sigmoid(b))  # GLU over channel axis
...
class ConformerLayer(nn.Module):
    ...
    def forward(
        ...
        att = self.self_attn(op, self.norm_self_att(op, residual), pos_emb, att_bias)
        residual = op.Add(residual, att)
        conv = self.conv(op, self.norm_conv(op, residual))
        residual = op.Add(residual, conv)
        residual = op.Add(
            residual,
            op.Mul(self.feed_forward2(op, self.norm_feed_forward2(op, residual)), fc),
        )
        return self.norm_out(op, residual)
