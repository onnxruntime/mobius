...
class _FalconTextModel(nn.Module):
    ...
    def __init__(self, config: ArchitectureConfig):
        super().__init__()
        self._alibi = config.alibi is not None
        self._dtype = config.dtype
        self._num_heads = config.num_attention_heads

        self.word_embeddings = Embedding(config.vocab_size, config.hidden_size)

        parallel_attn = config.alibi.parallel_attn if config.alibi is not None else False
        dual_ln = config.alibi.dual_ln if config.alibi is not None else False
...
class BloomCausalLMModel(FalconCausalLMModel):
    ...
    def __init__(self, config: ArchitectureConfig):
        ...
        config = dataclasses.replace(config, alibi=AlibiConfig())
        super().__init__(config)
...
class MPTCausalLMModel(FalconCausalLMModel):
    ...
    def __init__(self, config: ArchitectureConfig):
        ...
        config = dataclasses.replace(config, alibi=AlibiConfig(parallel_attn=False))
        super().__init__(config)
