...
class TestVibeVoiceASRStreaming:
    ...
    def test_stage_contract_and_explicit_state(self):
        ...
        module = VibeVoiceASRStreamingForConditionalGeneration(config)
    ...
    def test_cuda_fp16_excludes_prefix_only_gqa_fusion(self):
        config = dataclasses.replace(_streaming_config(), dtype=ir.DataType.FLOAT16)
        package = build_from_module(
            VibeVoiceASRStreamingForConditionalGeneration(config),
            config,
    ...
    def test_asr_config_rejects_non_executable_tokenizer_variants(self):
        from mobius._configs.vibevoice import _asr_streaming_tokenizer_config

        with pytest.raises(ValueError, match="mixer_layer='conv'"):
            _asr_streaming_tokenizer_config(
...
def _require_pinned_reference():
    ...
    if direct_url is not None:
        ...
        if vcs_info.get("commit_id") != VIBEVOICE_ASR_STREAMING_SOURCE_REVISION:
            pytest.skip(
                "Synthetic VibeVoice ASR parity requires "
                f"VibeVoice@{VIBEVOICE_ASR_STREAMING_SOURCE_REVISION}."
            )
    ...
    try:
        ...
        pytest.skip(
            ...
            f"VibeVoice@{VIBEVOICE_ASR_STREAMING_SOURCE_REVISION} in direct_url.json."
        )
    if source_revision != VIBEVOICE_ASR_STREAMING_SOURCE_REVISION:
        pytest.skip(
            "Synthetic VibeVoice ASR parity requires "
            f"VibeVoice@{VIBEVOICE_ASR_STREAMING_SOURCE_REVISION}."
        )
...
def test_vibevoice_asr_synthetic_two_chunk_prefill_and_cached_decode_parity():
    ...
    config = VibeVoiceASRStreamingConfig(
        hidden_size=16,
        intermediate_size=32,
        num_hidden_layers=1,
        num_attention_heads=2,
