...
def test_encoder_io_shapes():
    ...
    assert [i.name for i in sess.get_inputs()] == ["audio_signal"]
    t = 50
    feats = np.random.randn(1, config.fastconformer_feat_in, t).astype(np.float32)
    (out,) = sess.run(None, {"audio_signal": feats})
...
def test_encoder_supports_batch(batch):
    config, pkg = _build()
    sess = _session(pkg["encoder"])
    feats = np.random.randn(batch, config.fastconformer_feat_in, 40).astype(np.float32)
    (out,) = sess.run(None, {"audio_signal": feats})
